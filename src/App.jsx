...
export default function App() {
  const [activeTab, setActiveTab] = useState('media'); // 'media', 'srt', 'voice', 'history', 'privacy', 'guide', 'admin'
  ...
  return (
    <ActivationGuard>
      ...
      <div className="app-container">
        ...
        <div className="title-bar" onClick={handleTitleClick} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}>
          TOOLLANG • MEDIA STUDIO
          {isAdmin && (
            <span style={{ fontSize: '10px', background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b', padding: '2px 8px', borderRadius: '4px', border: '1px solid rgba(245, 158, 11, 0.2)', fontWeight: 'bold' }}>
              ADMIN MODE
            </span>
          )}
        </div>
          ...
          {isAdmin && (
            <div style={{ marginTop: '20px', padding: '15px' }}>
                <div 
                    ...
                    style={{ border: '1px dashed #f59e0b', background: 'rgba(245, 158, 11, 0.05)' }}
                >
                    <ShieldAlert size={20} color="#f59e0b" />
                    <span style={{ color: '#f59e0b', fontWeight: 'bold' }}>Admin Dashboard</span>
                </div>
            </div>
          )}
        </aside>
